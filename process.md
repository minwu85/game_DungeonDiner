...
## 12. Session 9 — Dialogic addon was actually already here, just broken; talk-or-buy at the shop

### 12.1 What "debug" / "FIX the bug" turned out to mean
The project already had a `dialogic-main/` folder under `addons/` - a full download of the [dialogic-godot/dialogic](https://github.com/dialogic-godot/dialogic) addon, extracted from a GitHub ZIP without moving its inner `addons/dialogic/` folder up to `res://addons/dialogic/`. `project.godot` was pointing `[editor_plugins] enabled` at the wrong nested path (`res://addons/dialogic-main/addons/dialogic/plugin.cfg`), and there was already a leftover `dialogic_default_action` input binding from a prior activation attempt - so this had clearly been tried before and hadn't worked. Since Dialogic's own scripts load their own resources via hardcoded `res://addons/dialogic/...` paths internally, having the real files one level too deep breaks it regardless of what `enabled` points at.

Fixed by moving `addons/dialogic-main/addons/dialogic` → `addons/dialogic` (so the addon lives where its own code expects it to), deleting the now-empty `dialogic-main` wrapper (just the repo's `LICENSE`/`README`/its own `Tests/` folder - not part of the addon itself), and correcting `project.godot`'s plugin path.

**This session's editor was open and live while this happened** - `project.godot` visibly changed out from under two edits mid-session (a `Dialogic="*uid://..."` autoload appeared, a `[dialogic]` config section appeared referencing the leftover `Tests/` folder, and the `enabled` list temporarily listed *both* the old and new plugin paths). That's the Godot editor's own plugin-activation code reacting live to the addon now being in the right place - good confirmation the move worked - but it meant re-reading and re-applying a couple of edits after the file changed under me. Cleaned up the transient duplicate/stale entries (a duplicate `Dialogic=` autoload line, the stale `Tests/` directory reference, the leftover old plugin path) to match what the addon's own activation code settled on.

### 12.2 Why the shop's dialogue is still hand-built, not a Dialogic timeline
With the addon now correctly installed and confirmed active (the editor resolving its `DialogicGameHandler.gd` script's UID live is about as much external verification as this environment can get), actually *using* it for the shopkeeper conversation would mean authoring a `.dtl` timeline in Dialogic's own script format and relying on its default runtime layout - a resource format and a rendering path I have no way to preview or validate here, on top of an addon that was JUST fixed and hasn't been exercised yet. That's a lot of new, unverifiable surface to add in the same pass as the fix itself. Chose not to: the mockup's actual requirement (a talk/buy choice, a short scripted exchange, name + portraits + textbox at the bottom) is fully met by a small addition to the existing hand-built shop UI, using the same Panel/Label/Button approach already proven to work everywhere else in this project this session.

The addon is there and enabled for whenever it's wanted - opening the project in the editor now should show a "Dialogic" tab in the bottom panel where a real timeline can be authored and previewed directly (which is the one part of this whole toolchain that's actually made to be visually checked while building it, unlike everything else here). Swapping the "Talk" button over to call `Dialogic.start("timeline_name")` instead of the custom dialogue box would be a small, contained change once a timeline exists.

### 12.3 Talk-or-buy at the shop
`cliff_side.gd`/`cliff_side.tscn`: pressing `Z` near the shop now opens a **ChoicePanel** ("Talk" / "Buy" / "Cancel") instead of jumping straight to the buy screen. **Buy** opens the existing purchase panel unchanged. **Talk** opens a new **DialogueBox** - a bottom bar with a name chip and text (matching the mockup's layout), a rounded portrait on the left showing the player's own sprite (same idle-frame technique used for the HUD/inventory portraits) and a plain gray circle on the right for the shopkeeper (no attempt to crop a face out of the shop's building spritesheet - confirmed back in Session 5 that these sheets aren't safely cropped blind, and the mockup itself only shows a generic gray circle, so this matches it faithfully rather than cutting a corner). A **Next** button steps through three short lines and closes on the last one.

Replaced the panel-visibility logic with a small `_any_panel_open()`/`_close_all_panels()` pair so the choice, buy, and dialogue panels can't overlap or get left open in an inconsistent state (e.g. walking out of shop range now reliably closes whichever of the three is open, where before only the buy panel was ever considered).
